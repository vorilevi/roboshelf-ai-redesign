# %% [markdown]
# # Roboshelf — T1 VLA Eval v2
# **Notebook neve: roboshelf-t1-eval-v1**
# **Input: notebook7b97304d37 (tréning notebook outputja)**
...
# %% [markdown]
# ## Cella 2 — Repók + mujoco_menagerie
...
# --- mujoco_menagerie (booster_t1 mesh fájlok) ---
# FONTOS: --filter=blob:none NÉLKÜL → ténylegesen letöltődnek az STL fájlok
if not MENAGERIE_DIR.exists():
    print("mujoco_menagerie klónozása (sparse: booster_t1)...")
    subprocess.run(["git", "clone", "--depth=1", "--sparse",
        "https://github.com/google-deepmind/mujoco_menagerie.git",
        str(MENAGERIE_DIR)], check=True)
    subprocess.run(["git", "sparse-checkout", "set", "booster_t1"],
        cwd=str(MENAGERIE_DIR), check=True)

stl_count = len(list(MENAGERIE_DIR.glob("booster_t1/assets/*.stl")))
print(f"  STL fájlok: {stl_count}")
assert stl_count > 0, "❌ STL fájlok hiányoznak — mujoco_menagerie clone sikertelen!"
print("  mujoco_menagerie ✅")

# --- ACTION_DIM patch (T1: 4) ---
DIM_PATTERNS = [
...
if not _ckpt_candidates:
    ...
    for d in sorted(KAGGLE_INPUT.iterdir()):
        ...
        for f in sorted(d.rglob("*"))[:10]:
            print(f"    {f}")
    raise FileNotFoundError("Add hozzá a notebook7b97304d37 outputját inputként!")
...
print(f"T1 Env OK — obs shape: {_obs.shape} ✅")
del _env_test
...
print(f"\n{'═'*65}")
print(f"EREDMÉNY — T1 UnifoLM-VLA-0")
