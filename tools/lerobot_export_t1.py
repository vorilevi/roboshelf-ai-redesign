"""
T1 push demo → LeRobot v3.0 dataset konverter.

Input:
    results/demos/t1_push_demos_1000.npz
...
Futtatás (repo gyökeréből):
    python3 tools/lerobot_export_t1.py
    python3 tools/lerobot_export_t1.py --in results/demos/t1_push_demos_1000.npz
    python3 tools/lerobot_export_t1.py --out data/lerobot/t1_push_v1 --all-episodes

Referenciák:
    G1 exporter:  tools/lerobot_export.py
    T1 env:       src/roboshelf_ai/mujoco/envs/manipulation/t1_shelf_stock_env.py
    Kaggle train: notebooks/kaggle_unifolm_vla_finetune.py
"""
...
DEFAULT_IN  = _REPO_ROOT / "results/demos/t1_push_demos_1000.npz"
DEFAULT_OUT = _REPO_ROOT / "data/lerobot/t1_push_v1"

MANIP_HZ   = 20
OBS_DIM    = 24
ACTION_DIM = 4   # T1: 4-DOF right arm (vs G1: 5)
...
def export_to_lerobot(
    ...
    """
    T1 NPZ demos → LeRobot v3.0 Parquet dataset.

    Args:
        npz_path:     bemeneti NPZ fájl
        out_dir:      kimeneti könyvtár (létrehozza ha nem létezik)
        success_only: csak sikeres epizódokat exportál (ajánlott)
    ...
    obs_all      = data["obs"]           # (N,) object array
    actions_all  = data["actions"]       # (N,) object array
    rewards_all  = data["rewards"]       # (N,) object array
    successes    = data["successes"]     # (N,) bool
    ...
    # --- Parquet írás + stat gyűjtés ---
    all_obs:     List[np.ndarray] = []
    all_actions: List[np.ndarray] = []
    episodes_meta = []

    for out_idx, src_idx in enumerate(indices):
        obs_ep    = obs_all[src_idx].astype(np.float32)     # (T, 24)
        action_ep = actions_all[src_idx].astype(np.float32) # (T, 4)
        reward_ep = rewards_all[src_idx].astype(np.float64) # (T,)
        T         = len(reward_ep)

        # done: utolsó lépés True
        done_ep = np.zeros(T, dtype=bool)
        done_ep[-1] = True

        # success: az egész epizód
    ...
    # --- Statisztikák ---
    all_obs_np    = np.concatenate(all_obs,    axis=0)  # (N_frames, 24)
    all_action_np = np.concatenate(all_actions, axis=0) # (N_frames, 4)
    ...
    # --- info.json (Kaggle notebook által olvasott mezők) ---
    info = {
        "dataset_name":   "roboshelf_t1_push_v1",
        "robot":          "booster_t1",
        "task":           TASK_LANG,
        ...
        "cameras":        [],          # nincs kamera — state-only
    ...
    if verbose:
        ...
        print(f"\nKövetkező lépés:")
        print(f"   HF push: huggingface-cli upload <repo> {out_dir}")
...
def main():
    parser = argparse.ArgumentParser(
        description="T1 push demo NPZ → LeRobot v3.0 dataset"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Sikertelen epizódokat is exportál (default: csak sikeres)"
    )
