"""
GR1T1 push task demo gyűjtő — scripted expert V2-ÁG (merev test).

⚠️ EZ EGY KÜLÖN ÁG. A tools/scripted_expert_gr1.py VÁLTOZATLAN marad.
...
Referenciák:
  Env:      src/roboshelf_ai/mujoco/envs/manipulation/gr1_shelf_stock_env_v2.py
  Scene:    src/envs/assets/scene_manip_sandbox_gr1_v2.xml
...
# 2026-08-01 — a PUSH_ARM_POS MEGEGYEZIK az env reset-pózával (_DEFAULT_ARM_POS).
# Nincs többé külön "hova álljunk" fázis: a kar már tolásra kész pózban indul.
PUSH_ARM_POS = np.array([-0.750, 0.000, 1.400, 0.300], dtype=np.float32)

SETTLE_STEPS = 1      # 2026-08-01: 80 → 1.
                      # A régi 80 NEM fizikai követelmény volt, hanem egy
                      # tranziens eltalálása. A passzív ízületek (láb, derék,
                      # csukló) rugóerő nélkül folyamatosan roskadtak, ezért a
                      # kéz z-je sosem állt be: t=20: 0.826 → t=60: 0.800 →
                      # t=120: 0.764 m. A 80. lépés kapta el azt a pillanatot,
                      # amikor a kéz épp a stock sávjában járt (0.787 m).
                      # Ezért volt a push-trigger nem-megfigyelhető (belső
                      # lépésszámláló) → a VLA-nak tanulhatatlan → 20% eval SR.
                      # A scene passive_joint stiffness-fixe (100000) után a kar
                      # valódi egyensúlyba kerül, és a reset-póz maga a push-póz.
                      # A megmaradt 1 lépés csak a kontakt előtti sanity-frame.
SETTLE_GAIN  = 5.0    # (nem használt — megtartva a régi hívások kompatibilitásáért)
PUSH_GAIN    = 5.0    # Jacobian transpose gain a push fázisban
MAX_DELTA_Q  = 0.018  # 2026-08-01: 0.10 → 0.018.
                      # Stabil karral a push lassítható az erő elvesztése nélkül.
                      # Mérés (100 ep): dq=0.10 → 4.0 frame/ep · dq=0.05 → 5.4 ·
                      # dq=0.018 → 17.4 frame/ep, 100% SR, 0 timeout ·
                      # dq=0.015 → 81% SR (már túl lassú). A 17.4 a T1
                      # referenciájához (22.9 frame/ep) illeszkedik.
...
class ScriptedPushController:
    ...
    def compute_action(
        ...
        # SETTLE: a kar már a push-pózban indul (reset-póz == PUSH_ARM_POS),
        # ezért ez 1 lépés, és egyszerűen tartja a pózt.
        # A régi J-transpose Cartesian szervó ELTÁVOLÍTVA: mérés szerint sosem
        # konvergált (|hand−cél| t=10: 0.026 m → t=100: 0.058 m, elsodródva),
        # és az akciói epizódonként reprodukálhatatlanok voltak — az epizódok
        # közötti szórás azonos t-nél (0.0162) megegyezett a lépésenkénti
        # akcióváltozással (0.0162), vagyis zaj volt, nem tanulható jel.
...
def main():
    ...
    out_path = Path(args.out) if args.out else \
        _REPO_ROOT / f"results/demos/gr1_push_demos_v2_{time.strftime('%Y%m%d_%H%M')}.npz"
