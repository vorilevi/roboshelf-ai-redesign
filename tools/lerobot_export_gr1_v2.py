"""
GR1T1 push demo → LeRobot v3.0 konverter — V2-ÁG (merev test).

⚠️ EZ EGY KÜLÖN ÁG. A tools/lerobot_export_gr1.py VÁLTOZATLAN marad.
...
DEFAULT_IN  = _REPO_ROOT / "results/demos/gr1_push_demos_v2_rigid.npz"
DEFAULT_OUT = _REPO_ROOT / "data/lerobot/gr1_push_v2"
...
SETTLE_SKIP = 0    # 2026-08-01: a paraméter GYAKORLATILAG OKAFOGYOTT.
                   # A scripted expert SETTLE_STEPS-e 80 → 1 lett (a scene
                   # passive_joint stiffness-fixe után a kar a push-pózban indul),
                   # így nincs mit kihagyni: minden frame célirányos tolás.
                   # Mért: 17.8 frame/ep, push-frame arány ~94%, konstans akció 0%.
                   #
                   # Történeti megjegyzés — NE állítsd vissza 75-re:
                   #   A SETTLE_SKIP=75 azért került be, hogy a 80 lépéses settle
                   #   konstans frame-jeit kidobja (push-arány ~4% → ~37%). De így
                   #   a model soha nem látta a start-pózt → evalban ott kezdett →
                   #   20-24% SR. A valódi hiba nem az arány volt, hanem hogy a
                   #   settle egy nem-megfigyelhető belső számlálóra várt.
                   #   Részletek: [[kiserlet_agent_asszisztalt_robot_portolas]] 5.7
...
def export_to_lerobot(
    ...
    info = {
        "dataset_name":    "roboshelf_gr1_push_v2",
        "robot":           "fourier_gr1t1",
        "task":            TASK_LANG,
    ...
    if verbose:
        ...
        print(f"\nKövetkező lépés:")
        print(f"   HF push: huggingface-cli upload vorilevi/roboshelf-gr1-push-v2 {out_dir} --repo-type dataset")
