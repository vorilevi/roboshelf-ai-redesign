"""
GR1 Shelf Stocking Push Env — v2 (Track 3, MEREV-TEST ÁG).

⚠️ EZ EGY KÜLÖN ÁG. A v1 (gr1_shelf_stock_env.py) VÁLTOZATLAN marad, mert
   azon fut a korábbi betanítás (20% eval SR baseline). A két ág nem
   hivatkozik egymásra, és külön scene / expert / export / dataset tartozik
   hozzájuk. Részletek: [[kiserlet_agent_asszisztalt_robot_portolas]] 5.8
...
_SCENE_XML = _REPO_ROOT / "src/envs/assets/scene_manip_sandbox_gr1_v2.xml"
...
# 2026-08-01 FIX — a reset-póz EGYBEN a push-póz (T1-minta).
#   Régi: [-1.000, 0.000, 0.500, 0.083] + külön PUSH_ARM_POS a scripted expertben,
#   amit egy 80 lépéses "settle" hidalt át. A settle valójában nem beállás volt,
#   hanem egy tranziens eltalálása: a passzív ízületek rugóerő nélkül folyamatosan
#   roskadtak, és a 80. lépés kapta el azt a pillanatot, amikor a kéz épp a stock
#   magassági sávjában járt. Emiatt a push-trigger nem volt megfigyelhető
#   (belső lépésszámláló) → a VLA-nak tanulhatatlan feladat → 20% eval SR.
#   A scene passive_joint stiffness-fixe után a kar valódi egyensúlyba kerül,
#   így a reset-póz beállítható közvetlenül a tolásra kész pózra, és a
#   settle-fázis elhagyható. Minden frame célirányos mozgás (G1/T1-minta).
#   Mért egyensúlyi kéz-pozíció: (0.376, -0.267, 0.815), tranziens 5.9 mm.
_DEFAULT_ARM_POS = np.array([
    -0.750,  # right_shoulder_pitch
     0.000,  # right_shoulder_roll
     1.400,  # right_shoulder_yaw
     0.300,  # right_elbow_pitch
], dtype=np.float32)
...
STOCK_X_FIXED   = 0.36    # tároló asztal x pozíciója (2026-08-01: 0.45→0.36,
                          # a merev-test fix után a kar elérése a stock magassági
                          # sávjában max ~0.39m; a 0.45 a MEGERESZKEDETT kar mérése volt)
